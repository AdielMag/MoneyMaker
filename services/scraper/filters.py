...
class MarketFilter:
    ...
    def _check_time_to_resolution(self, market: Market) -> FilterResult:
        ...
        if time_to_resolution > max_hours:
            market.passes_filter = False
            market.filter_reason = f"Time to resolution ({time_to_resolution:.1f}h) exceeds maximum ({max_hours}h)"
            return FilterResult(
                passed=False,
                market=market,
        ...
        if time_to_resolution < min_hours:
            market.passes_filter = False
            market.filter_reason = f"Market resolves too soon ({time_to_resolution * 60:.0f} minutes)"
            return FilterResult(
                passed=False,
                market=market,
    ...
    def _check_volume(self, market: Market) -> FilterResult:
        ...
        if market.volume < min_volume:
            market.passes_filter = False
            market.filter_reason = f"Volume (${market.volume:,.0f}) below minimum (${min_volume:,.0f})"
            return FilterResult(
                passed=False,
                market=market,
    ...
    def _check_liquidity(self, market: Market) -> FilterResult:
        ...
        if market.liquidity < min_liquidity:
            market.passes_filter = False
            market.filter_reason = f"Liquidity (${market.liquidity:,.0f}) below minimum (${min_liquidity:,.0f})"
            return FilterResult(
                passed=False,
                market=market,
    ...
    def _check_price_range(self, market: Market) -> FilterResult:
        ...
        market.passes_filter = False
        market.filter_reason = f"All outcome prices are extreme (outside {min_price:.0%}-{max_price:.0%})"
        return FilterResult(
            passed=False,
            market=market,
